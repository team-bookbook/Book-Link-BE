...
import com.bookbook.booklink.chat.chat_mutual.service.ChatMessagesService;
...
public class OneToOneChatsService {
    ...
    public SingleRoomResDto getOrCreateChatRoom(SingleRoomReqDto dto) {
        ...
        Optional<OneToOneChats> existingRoom = oneToOneChatsRepository.findByUser1IdAndUser2Id(dto.getUser1Id(),dto.getUser2Id());
        if (existingRoom.isEmpty()) {
            existingRoom = oneToOneChatsRepository.findByUser2IdAndUser1Id(dto.getUser1Id(),dto.getUser2Id());
        }
        // 있으면 반환, 둘다 없는 경우 새로 생성
        OneToOneChats chat = existingRoom.orElseGet(() ->
                // 새로 생성
                oneToOneChatsRepository.save(OneToOneChats.createChatRoom(dto.getUser1Id(), dto.getUser2Id()))
        );
